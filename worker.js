...
go.onclick=async()=>{go.disabled=true;error.innerHTML="";advisory.innerHTML="";results.innerHTML="";status.textContent=" Fetching current online sources…";try{let d=await call("/api/market",{sources:[...sel]});status.textContent=" "+d.items.length+" current signals found";results.innerHTML="<div class=card><div class=label>Live intelligence</div>"+d.items.map((x,i)=>"<div class=news><b>"+esc(x.source||"")+"</b> · "+esc(x.date||"recent")+"<h3>"+esc(x.title||"")+"</h3><p>"+esc(x.snippet||"")+"</p>"+(x.url?"<a class=link target=_blank href=\""+esc(x.url)+"\">Open source →</a>":"")+" <button class=btn data-i=\""+i+"\">Analyse</button></div>").join("")+"</div>";results.querySelectorAll("button[data-i]").forEach(b=>b.onclick=()=>analyse(d.items[+b.dataset.i]))}catch(e){error.innerHTML="<div class=err>"+esc(e.message)+"</div>"}finally{go.disabled=false}};
